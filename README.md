...
The examples below use version `0.1.0` — substitute the version you downloaded.

| Platform            | File                                      |
|---------------------|-------------------------------------------|
| Linux x86_64        | `rpg-mcp-server-0.1.0-linux-x86_64`       |
| Linux aarch64       | `rpg-mcp-server-0.1.0-linux-aarch64`      |
| macOS Apple Silicon | `rpg-mcp-server-0.1.0-macos-aarch64`      |
| Windows x86_64      | `rpg-mcp-server-0.1.0-windows-x86_64.exe` |
...
```bash
chmod +x rpg-mcp-server-0.1.0-*
```

On macOS, also clear the quarantine flag the first time:

```bash
xattr -d com.apple.quarantine rpg-mcp-server-0.1.0-macos-aarch64
```

> Prefer the JVM? Download `rpg-mcp-server-0.1.0-runner.jar` instead and run it with
> [Java 21+](https://adoptium.net/): the command becomes `java` and the arguments start with
> `-jar /path/to/rpg-mcp-server-0.1.0-runner.jar`. Everything else below is identical.
...
```bash
claude mcp add rpg --scope user -- /path/to/rpg-mcp-server-0.1.0-linux-x86_64 -Dquarkus.mcp.server.stdio.enabled=true
```
...
```json
{
  "mcpServers": {
    "rpg": {
      "command": "/path/to/rpg-mcp-server-0.1.0-linux-x86_64",
...
```bash
codex mcp add rpg -- /path/to/rpg-mcp-server-0.1.0-linux-x86_64 -Dquarkus.mcp.server.stdio.enabled=true
```
...
```json
{
  "mcpServers": {
    "rpg": {
      "command": "C:\\Users\\YourName\\path\\to\\rpg-mcp-server-0.1.0-windows-x86_64.exe",
      "args": [
...
The two extra arguments are a Windows workaround: Claude Desktop may launch the server with
`C:\WINDOWS\system32` as the working directory, which Quarkus cannot scan. Point `-Duser.dir` at any
directory you can write to. Restart Claude Desktop afterwards.
